...
#exp_name <- "submission_final_norejects_dada_final_camelia" ; orgn <- "fungi"
...
save_img <- paste0(path_to_taxa, "figures_improved/", taxa_algo, "/")
...
# select all params
all_available_params <- c("pH", "SM", "C", "N", "CN", "SOM", "LOI", "TOC", "TN", "DON", "NH4", "NO3", "PO4", "d15N", "d13C", "microC", "microN", "microCN", "nirS", "nosZ", "nifH", "mcrA", "mxaF")
...
interesting_params[["qpcr"]] <- c("nirS", "nosZ", "nifH", "mcrA", "mxaF") ; plot_cols[["qpcr"]] <- 5
...
for (intp in names(interesting_params)) {
...
#	intp <- "final"
	...
	export_svg(paste0(save_scatter, "standard_phylo_by_timepoint_", intp), scatter_time, width=72, height=84, base=FALSE)

	scatter_treat <- ggplot_pairs(no_nas, interesting_params[[intp]], "Treatment", n_cols=plot_cols[[intp]], normalised=T)

	export_svg(paste0(save_scatter, "standard_phylo_by_treatment_", intp), scatter_treat, width=72, height=84, base=FALSE)
}
...
# export
export_svg(paste0(save_corr, "new_correlogram_pearson_", exp_name, "_easy_to_read"), final_corr, base=FALSE)
...
## add plants for the statistics
#all_available_and_plants <- c(all_available_params, "Deciduous_shrub", "Evergreen_shrub", "Lichen", "Forb", "Litter", "plant_diversity", "plant_eveness")

sink(paste0(save_box, "all_enviro_stats_no_interaction_refs_C_July.txt"))
...
#	an_env <- all_available_params[1]
	...
	# print
	print(paste0("Analysing ", toupper(an_env)))
	print(summary(an_env_model))
...
# select all params
not_all_available_params <- c("pH", "SM", "C", "N", "CN", "SOM", "LOI", "TOC", "TN", "DON", "NH4", "NO3", "PO4", "d15N", "d13C", "microC", "microN", "microCN", "nirS", "nosZ", "nifH", "mcrA", "mxaF")
...
pb <- print_boxplots_paired(long_tab_env, not_all_available_params, exp_name, save_box_comparison_env)
...
for (an_env in all_available_params) {
	...
	tmp_multiplot <- ggplot(subset[!is.na(subset$norm_value), ], aes(x=TimePoint, y=norm_value, fill=Treatment)) +
			...
			ggplot_theme(leg_pos="bottom", ang_le=60, fnt_sz=70)
...
all_together <- cowplot::plot_grid(
			cowplot::plot_grid(
					...
					the_legend,
					ncol=6, rel_widths=c(1,1.3,1,1,1,1)
				),
			cowplot::plot_grid(
...
# export
export_svg(paste0(save_box_comparison_env, "all_together_", exp_name), all_together, height=60, width=60)
...
# export
export_svg(paste0(save_box_comparison_env, "no_gene_all_together_", exp_name), all_together_no_genes, height=60, width=60)
...
#	a_treat <- "Control" ; an_env <- "pH"
	...
	# plot normalised
	dodge <- position_dodge(width = 0.8)
	plt_all <- ggplot(env_by_treat[which(env_by_treat$variable %in% not_all_available_params), ], aes(x=TimePoint, y=norm_value, fill=TimePoint)) +
		geom_violin(width=1, position=dodge, linewidth=1) +
		geom_boxplot(width=0.2, color="black", position=dodge, linewidth=1, show.legend=FALSE) +
	...
	# export
	export_svg(paste0(save_box_comparison_env, "all_enviro_comparison_by_time_", a_treat, "_samples_", exp_name), plt_all)
...
# list available environmental parameters
env_variables <- c("nirS", "nosZ", "nifH", "mcrA", "mxaF")

# empty var for putting all the data together, for plotting
long_tab_qpcr <- vector()

# reshape table for plotting with facets
for (a_gene in env_variables){
...
# print with paired version, Warming versus Control
pb <- print_boxplots_paired(long_tab_qpcr, env_variables, exp_name, save_box_comparison_qpcr)
...
# export
export_svg(paste0(save_box_comparison_qpcr, "all_genes_comparison_by_treatment_samples_", exp_name), trt_pl_qpcr, width=32, height=22)
...
#	a_treat <- "Control" ; a_gene <- "mcrA"
	...
	# export
	export_svg(paste0(save_box_comparison_qpcr, "all_genes_comparison_by_time_", a_treat, "_samples_", exp_name), plt_all)
...
surf_temp_data <- read.csv("/microbiology/disko2013/metadata/csv_original/surface_temp_analysed_timeframe.csv", header=T, sep=",")
...
timeline_surf <- ggplot(info_mean_surf, aes(x=day, y=mean_temp, color=treatment)) +
		...
		guides(color=guide_legend(override.aes=list(size=20, shape=15)), ncol=2) +
		ggplot_theme(leg_pos="right", ang_le=45)

# export
export_svg(paste0(save_box_temp, "surface_yearly_temperatures_time_line_", exp_name), timeline_surf)
...
#	stat_test$p.adj <- scientific(round(stat_test$p.adj, digits=8), digits=2)
...
temp_box_surf <- bxp_surf + stat_pvalue_manual(stat_test, label="p.adj", hide.ns = TRUE, size=15)

# export
export_svg(paste0(save_box_temp, "surface_yearly_temperatures_box_", exp_name), temp_box_surf, width=32, height=22)
...
five_deep_temp <- read.csv("/microbiology/disko2013/metadata/csv_original/five_cm_deep_temp_analysed_timeframe.csv", header=T, sep=",")
...
# export
export_svg(paste0(save_box_temp, "five_deep_yearly_temperatures_time_line_", exp_name), timeline_five)
...
# add stats to faceted boxplots
temp_box_five <- bxp_fived + stat_pvalue_manual(stat_test, label="p.adj", hide.ns = TRUE, size=15)

# export
export_svg(paste0(save_box_temp, "five_deep_yearly_temperatures_box_", exp_name), temp_box_five, width=32, height=22)
...
supplementary_fig_four_a <- plot_grid(temp_box_surf, temp_box_five, labels = c('A', 'B'), label_size = 100, ncol=1)

# export it
export_svg(paste0(save_final_figs, "supplementary_fig_four_a"), supplementary_fig_four_a, base=FALSE, width=48, height=36)

###################################################################################################

supplementary_fig_four_b <- plot_grid(timeline_surf, timeline_five, labels = c('A', 'B'), label_size = 100, ncol=1)

# export it
export_svg(paste0(save_final_figs, "supplementary_fig_four_b"), supplementary_fig_four_b, base=FALSE, width=48, height=36)
