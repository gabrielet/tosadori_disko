...
for (xna in names(phylo_xna)) {
		...
		for (which_tab in names(linda_outs$output)) {
			...
			if (any(volcano_data$log2FoldChange >= lfc_thresh & volcano_data$padj <= signif_t) | any(volcano_data$log2FoldChange <= -lfc_thresh & volcano_data$padj <= signif_t)) {
				...
				volcano <- ggplot(volcano_data, aes(x=log2FoldChange, y=log_p, label=taxa)) +
					...
					scale_fill_manual(values = c("up"="#E78AC3", "down"="#66C2A5", "other"= "gray50")) +
					geom_hline(yintercept=p_thresh, linetype="dashed", color = "gray30", linewidth=1) +
					geom_text_repel(size=15, nudge_x=-0.5, nudge_y=-0.5, max.overlaps=20) +
					ylab("-log10(p-value)") +
					xlab("log2FoldChange") +
					ggplot_theme(leg_pos="none", ang_le=0)

				# and export
				export_svg(paste0(save_diff_abundance, "linda_", withint, exp_name, "_", lbl, "_", pvaladj, "_WITH_TAXA_LEVS"), volcano, as_rds=T)
				...
				# plot
				bar_plot <- ggplot(significant_ones, aes(x=log2FoldChange, y=taxa, fill=threshold)) +
				...
				# and export
				export_svg(paste0(save_diff_abundance, "barplot_linda_", withint, exp_name, "_", lbl, "_", pvaladj, "_WITH_TAXA_LEVS"), bar_plot, as_rds=T)
			} else {
	...
	if (length(all_significant_only) > 0) {
		...
		# export table with volcano info
		write.table(all_significant_only_final, paste0(save_diff_abundance, "all_significant_only_", withint, exp_name, "_", lbl, "_", pvaladj, "_WITH_TAXA_LEVS.csv"), row.names=F, col.names=T, sep="\t", quote=F)
	}
}
...
for (taxa_level in interesting_levels) {
	...
	if (any(volcano_data$log2FoldChange >= lfc_thresh & volcano_data$padj <= signif_t) | any(volcano_data$log2FoldChange <= -lfc_thresh & volcano_data$padj <= signif_t)) {
		...
		# and export
		export_svg(paste0(save_diff_abundance, "linda_", exp_name, "_", lbl, "_", pvaladj), volcano, as_rds=T)
		...
		# plot
		bar_plot <- ggplot(significant_ones_xna, aes(x=log2FoldChange, y=taxa, fill=threshold)) +
				geom_bar(stat="identity", width=0.5, colour="black") +
				scale_fill_manual(values = c("up"="#E78AC3", "down"="#66C2A5", "other"= "gray50")) +
				ggtitle("") +
				ggplot_theme(leg_pos="none", ang_le=0) +
		...
		# and export
		export_svg(paste0(save_diff_abundance, "DNA_RNA_barplot_linda_", exp_name, "_", lbl, "_", pvaladj), bar_plot, as_rds=T)
...
all_significant_only_drna$t_names[grep("Unidentified", all_significant_only_drna$taxa, invert=T)] <- unlist(lapply(strsplit(all_significant_only_drna$taxa[grep("Unidentified", all_significant_only_drna$taxa, invert=T)], "__"), "[[", 2))
...
# export table with volcano info
write.table(all_significant_only_drna_final, paste0(save_diff_abundance, "all_significant_only_drna_", exp_name, "_", lbl, "_", pvaladj, ".csv"), row.names=F, col.names=T, sep="\t", quote=F)

print("LinDA analysis is done, export rds")

# export all the data thata were computed
saveRDS(all_res, paste0(save_diff_abundance, "linda_out_", ref_lev_time, "_", ref_lev_treat, "_treat_by_time_and_site_threshold_", lfc_thresh, "_", pvaladj, ".Rds"))
